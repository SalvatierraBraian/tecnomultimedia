...
int reinicioAreaX = 45; // Coordenada X del área de reinicio
int reinicioAreaY = 122; 
int reinicioAreaWidth = 540; // Ancho del área de reinicio 320,240,500,400
int reinicioAreaHeight = 230; 
void setup() {
  size(640, 480);
...
void draw() {
 ...
 void mousePressed(){
 if (mouseX >= reinicioAreaX && mouseX <= reinicioAreaX + reinicioAreaWidth &&
      mouseY >= reinicioAreaY && mouseY <= reinicioAreaY + reinicioAreaHeight) { 
         estado = 0;
       tiempo = 0;
       millis();
       indiceTexto = 0;
